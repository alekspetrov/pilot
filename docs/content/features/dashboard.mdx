...
The [Desktop App](/deployment/desktop-app) connects to the same HTTP gateway via the `/health` endpoint for service discovery. When Pilot is running locally, the desktop app auto-detects it and displays the same panels as the web dashboard in a native macOS window.
