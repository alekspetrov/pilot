...
Without Navigator, Pilot would generate generic code. With Navigator, it generates code that follows your established conventions. Learn more about [how Navigator integrates with the execution pipeline](/concepts/how-it-works#navigator-integration).

### 3. Autopilot feedback loop

Most AI tools stop at code generation. Pilot manages the [full PR lifecycle](/concepts/how-it-works#execution-flow):
...
Not every ticket needs the same level of AI reasoning. Pilot [classifies ticket complexity](/concepts/model-routing) and routes accordingly:
...
**Not for security-critical code.** Authentication flows, encryption, access control — these need careful human review regardless of who writes them. Pilot can write tests for security code, but shouldn't be the primary author. See [Security](/concepts/security) for how Pilot handles its own authentication and cost controls.
...
- [How It Works](/concepts/how-it-works) — Architecture, execution pipeline, and autopilot state machine
- [Model Routing](/concepts/model-routing) — Complexity classification, model selection, and cost optimization
- [Security](/concepts/security) — Authentication, budget controls, and the alert system
